#!/bin/bash
# Скрипт автоматизации Модуля 1 (Полная версия)
...
if [ -z "$ROLE" ]; then
    echo "Использование: ./setup.sh [hq-srv|br-srv|hq-rtr|br-rtr|isp|hq-cli|hq-sw]"
    exit 1
fi
...
setup_users() {
    ...
    if [[ "$ROLE" == *"srv"* ]]; then
        adduser --gecos "" remote_user --disabled-password
        adduser --uid 2026 --gecos "" sshuser --disabled-password
    ...
    if [[ "$ROLE" == *"rtr"* ]]; then
        adduser --gecos "" net_admin --disabled-password
...
EOF
        echo "Установка и настройка Bind9..."
        apt install -y bind9
...
EOF
    ...
    "hq-sw")
        ...
        ovs-vsctl add-port br-hq ens33 trunks=100,200,999
...
echo "--- НАСТРОЙКА ЗАВЕРШЕНА. ПРОВЕРЬТЕ IP И СЛУЖБЫ ---"
